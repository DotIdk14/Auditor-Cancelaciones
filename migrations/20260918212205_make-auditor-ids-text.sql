-- Los campos de auditoría pasan a TEXT (identificador libre del auditor:
-- email o nombre) mientras no exista autenticación integrada. Cuando se
-- integre auth real se agregarán columnas UUID dedicadas con FK a auth.users.

-- 1) Se eliminan las policies de tickets que comparan created_by con auth.uid()
...
-- 1b) Se eliminan las FK a auth.users (bloquean el cambio de tipo a TEXT)
...
-- 2) Se alteran los tipos de columna
...
-- 3) Se recrean las policies de tickets comparando contra auth.uid()::text
...
-- 4) Se regeneran las funciones helper para comparar como texto
